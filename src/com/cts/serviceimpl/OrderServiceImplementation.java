...
public class OrderServiceImplementation implements OrderService {
	...
	@Override
	public void viewOrderById(int order_id) throws OrderSelectionException, OrderNotFoundException {
try {
	...
	if (order != null) {
		...
		throw new OrderNotFoundException("Sorry, No Orders not Found with the specified ID, Please search for any other order");
	}		
		} catch (OrderSelectionException e) {
			// TODO: handle exception
			System.out.println("Failed to view your Order due to an error. Please try again later.");
			e.printStackTrace();
		}
	...
	@Override
	public void viewOrder() throws OrderItemsSelectionException {
		try {
	...
	@Override
	public void deleteOrder() throws OrderSelectionException, OrderNotFoundException {
		try {
			...
			// TODO: handle exception
			System.out.print("Sorry, there is an Error deleting the Order. Please try again after sometime");
			e.printStackTrace();
	...
	@Override
	public void viewAllOrders() throws OrderNotFoundException {
try {List<Order> ordersList = new ArrayList<>();
...
if (ordersList != null) {
	...
	throw new OrderNotFoundException("Sorry, No Orders available now");
}
	...
	@Override
	public void addOrder() throws OrderSelectionException, OrderItemsSelectionException, OrderUpdationException, CustomerSelectionException, UpdateStockException, ProductSelectionException {
try {
	...
	do {
		...
		if (quantity > existingQuantityInStock) {
			System.out.print("Insufficient Stock for Item" + name + ", Available stock is" + existingQuantityInStock);
			continue;
		}
	...
	if (order != null) {
		...
		viewOrderitems(order_id);
			...
			// TODO: handle exception
			System.out.println("Failed to add your Order due to an error. Please try again later.");
			e.printStackTrace();
		}
		
	}
@Override
	public void viewOrderitems(int order_id) throws OrderSelectionException, OrderItemsSelectionException {
	List<OrderItemCart> orderItemCartsList = orderDao.viewOrderitems(order_id);
...
@Override
public void updateOrder() throws OrderSelectionException, OrderItemsSelectionException, OrderItemsUpdationException, ProductSelectionException, UpdateStockException, OrderItemsDeletionException {
	try {
		 ...
		 */
		Order existOrder = orderDao.existOrder(order_id);
		if (existOrder == null) {
		...
		switch(person)
		{
			...
			case "c":
				 ...
				 * delivered, in both the conditions, the user can't update the order.
				 * it is considered that the average delivery and shipping time period to
				 * be 7 and 4 days from the date of creating order, considering the time period of the order 
				 * to be updateable is maximum of 3 days, which can be calculated by deducing the 
				 * order date from the current date which is checked only post checking the shipment and delivery
				 */
				
				if (existingStatus.equalsIgnoreCase("shipped")){
					System.out.println(
							"This order is already " + existingStatus + " and cannot be updated.");
					return;
				}
				...
				Then, the Instant is converted to a ZonedDateTime by applying the system's default time zone.
				Finally, the ZonedDateTime is converted to a LocalDateTime by stripping away the time zone information.
				 */
	            ...
	            /* Chronounit- part of java.time 
	             *  Calculating the difference in days using ChronoUnit.
	             *  ChronoUnit.DAYS is an enumeration in the java.time package that represents a standard set of date and time units. 
	             *  in ChronoUnit DAYS specifically refers to the number of days.
	             */
				 ...
				 */
	            Map<Integer, Integer> existingItems=orderDao.selectOrderItem(order_id);
				/*
				...
				*/
				...
				do {
					...
					if (quantity == 0) 
					{
						...
						/*
						 * if a product is removed from order cart before shipping or delivery, its quantity in stock is increased to 
						 * the currently existing plus removed product's quantity from the cart
						 */
					} 
						...
						if (existProductinCart == null) {
							...
							/*
							 * new quantity needed must me less than already taken quantity plus currently available quantity in the stock
							 */
				...
				/*
				 * calculating the total amount of order all again instead of just adding the new added quantity
				 */
		...
		// TODO: handle exception
		System.out.println("Failed to Update your Order due to an error. Please try again later.");
		e.printStackTrace();
	}
		
	}
@Override
	public void cancelOrder() throws OrderUpdationException, UpdateStockException, OrderSelectionException {
	try {
